...
public interface IPlayerSchduleTaskSupport<P extends Player> extends GGServerSupport{
	 ...
	 * 2019-09-22 11:42:55
	 */
	Map<Object, GGFuture> getScheduleTaskFutures();
	...
	default void schedule(Object taskKey, long delayMs, TimeUnit timeUnit, Runnable runnable) {
		GGFuture taskFuture = getGGServer().schedule(delayMs, timeUnit, runnable);
	...
	default void cancelScheduleTask(Object taskKey) {
		GGFuture taskFuture = getScheduleTaskFutures().get(taskKey);
		if (taskFuture != null) {
