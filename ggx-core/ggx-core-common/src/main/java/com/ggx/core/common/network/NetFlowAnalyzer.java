...
public class NetFlowAnalyzer {
	...
	public NetFlowAnalyzer(GGXCoreConfig config) {
		this.config = config;
	...
	public void startNetFlowAnalyzeTask() {
		taskExecutor.schedule(SECONDS, TimeUnit.SECONDS, () -> {
			try {
				...
				GGXLogUtil.getLogger(this).error("Net Flow Analyze Task Error!", e);
			}
	 ...
	 * 2020-12-03 10:53:45
	 */
	public void analyzeDownFlow(long dataLength, GGXSession session) {
		try {
			NetFlowData netFlowData = this.getNetFlowData(session);
			netFlowData.incrTotalDown(dataLength);
			netFlowData.incrTotal(dataLength);
		} catch (Exception e) {
			GGXLogUtil.getLogger(this).error("Analyze Down Flow Error!", e);
		}
	}
	 ...
	 * 2020-12-03 10:46:42
	 */
	private NetFlowData getNetFlowData(GGXSession session) {
		NetFlowData netFlowData = session.getNetFlowData();
		if (netFlowData == null) {
